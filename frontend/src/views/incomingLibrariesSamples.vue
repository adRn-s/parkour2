        ...
        <div class="button-popup-wrapper">
          ...
          >
            <ul style="padding-left: 0px">
              <li
                v-for="(column, index) in columnsList.filter(
    ...
    <div class="table-container">
      <div style="margin: 10px; border: 1px solid lightgray">
        <TabulatorTable
          v-if="!loading"
          ref="tabulatorTable"
          :rowData="filteredLibrariesSamples"
          :columnDefs="currentVisibleColumns"
...
export default {
          ...
          return `
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                    ...
                    <span style="font-weight: normal; font-size: 12px;">(${count} item${
            count > 1 ? "s" : ""
  ...
  methods: {
      ...
      this.columnsList = [
        {
          ...
          formatter: function (cell) {
            ...
            const checkbox = `<input type="checkbox" style="top:-4px" ${
    ...
    handleGroupButtonClick(event, groupValue, action) {
      ...
      const tabulatorTable = this.$refs.tabulatorTable.getTable().value; // Access Tabulator instance
      if (!tabulatorTable) {
        console.error("Tabulator instance is not available");
        return;
      }

      console.log("ss", tabulatorTable);
      const group = tabulatorTable
        .getGroups()
        .find((g) => g.key === groupValue);
      if (!group) return;
    ...
    onSearch() {
      ...
      searchedLibrariesSamplesList = this.librariesSamplesList.filter(
        (row) =>
          (lowercasedQuery !== "" &&
...
.group-action-buttons-container {
  ...
  background-color: white; /* Prevent overlap from background elements */
  z-index: 10; /* Ensure it stays above other content */
  padding: 0 10px; /* Add some padding for spacing */
  border-left: 1px solid #ddd; /* Optional: adds a subtle separator */
}
